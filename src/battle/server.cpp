...
BattleServer* BattleServer::start() noexcept
{
    ...
    // start net thread to login server with blocking
    cli.start(true);
    ...
    sev.onConnection = [this](const hv::SocketChannelPtr& channel)
    {
        ...
        if (channel->isConnected())
        {
            ...
            if (auto& room = this->Battles[this->AddrMap.at(ip)]; 
                not room.ConnectMap.contains(channel->peeraddr()))
            {
                room.ConnectMap.emplace(channel->peeraddr(), channel);
            }
        }
        else
        {
    ...
    Battle::EventLoop.reset(new hv::EventLoopThread());
    Battle::EventLoop->start();
    Battle::EventLoop->join();
...
void BattleServer::init(std::string&& path) noexcept 
{
    ...
    std::call_once(flag, [&lua = BattleServer::Config, path = std::move(path)]() {
        ...
        Battle::RoundLongingTime = lua["Config"]["RoundLongingTime"].get<int>();
    });
}
